#include "../../frontier_exploration/include/frontier_exploration/bounded_explore_layer.h"
...
#include "../../frontier_exploration/include/frontier_exploration/frontier_search.h"
...
void BoundedExploreLayer::onInitialize() {
  ...
  polygonService_ =
      nh_.advertiseService("update_boundary_polygon", &BoundedExploreLayer::updateBoundaryPolygonService, this);
  frontierService_ = nh_.advertiseService("get_next_frontier", &BoundedExploreLayer::getNextFrontierService, this);

  dsrv_ = new dynamic_reconfigure::Server<frontier_exploration::BoundedExplorePluginConfig>(nh_);
  dynamic_reconfigure::Server<frontier_exploration::BoundedExplorePluginConfig>::CallbackType cb =
      boost::bind(&BoundedExploreLayer::reconfigureCB, this, _1, _2);
...
void BoundedExploreLayer::reconfigureCB(frontier_exploration::BoundedExplorePluginConfig& config, uint32_t level) {
  enabled_ = config.enabled;
  cost_threshold_ = (unsigned char) config.cost_threshold;
}

bool BoundedExploreLayer::getNextFrontierService(frontier_exploration::GetNextFrontier::Request& req,
                                                 frontier_exploration::GetNextFrontier::Response& res) {
  return getNextFrontier(req.start_pose, res.next_frontier);
}

bool BoundedExploreLayer::getNextFrontier(geometry_msgs::PoseStamped start_pose,
  ...
  std::list<Frontier> frontier_list = frontierSearch.searchFrom(start_pose.pose.position, cost_threshold_);
  ...
  quant.setRPY(0, 0, yawOfVector(start_pose.pose.position, next_frontier.pose.position));
  tf2::convert(quant, next_frontier.pose.orientation);
  return true;
}

bool BoundedExploreLayer::updateBoundaryPolygonService(frontier_exploration::UpdateBoundaryPolygon::Request& req,
...
void BoundedExploreLayer::updateBounds(double robot_x, double robot_y, double robot_yaw, double* min_x, double* min_y,
...
void BoundedExploreLayer::mapUpdateKeepObstacles(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i,
                                                 int max_j) {
  if (!enabled_)
    return;
  ...
  for (int j = min_j; j < max_j; j++) {
    ...
    for (int i = min_i; i < max_i; i++) {
      // only update master grid if local costmap cell is lethal/higher value, and is not overwriting a lethal
      // obstacle in the master grid
