...
#include "../../frontier_exploration/include/frontier_exploration/costmap_tools.h"
...
std::list<Frontier> FrontierSearch::searchFrom(geometry_msgs::Point position, unsigned char cost_threshold) {
  ...
  if (nearestCell(clear, pos, cost_threshold, costmap_)) {
    bfs.push(clear);
  } else {
    bfs.push(pos);
    ROS_ERROR("Could not find nearby clear cell to start search");
  }
  ...
  while (!bfs.empty()) {
    ...
    BOOST_FOREACH (unsigned nbr, nhood4(idx, costmap_)) {
      ...
      if (map_[nbr] <= cost_threshold && !visited_flag[nbr]) {
        visited_flag[nbr] = true;
        bfs.push(nbr);
        // check if cell is new frontier cell (unvisited, NO_INFORMATION, free neighbour)
      } else if (isNewFrontierCell(nbr, cost_threshold, frontier_flag)) {
        frontier_flag[nbr] = true;
        Frontier new_frontier = buildNewFrontier(nbr, pos, cost_threshold, frontier_flag);
...
Frontier FrontierSearch::buildNewFrontier(unsigned int initial_cell, unsigned int reference,
                                          unsigned char cost_threshold, std::vector<bool>& frontier_flag) {
  ...
  while (!bfs.empty()) {
    ...
    BOOST_FOREACH (unsigned int nbr, nhood8(idx, costmap_)) {
      ...
      if (isNewFrontierCell(nbr, cost_threshold, frontier_flag)) {
...
bool FrontierSearch::isNewFrontierCell(unsigned int idx, unsigned char cost_threshold,
